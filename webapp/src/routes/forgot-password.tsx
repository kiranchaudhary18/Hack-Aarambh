...
export const Route = createFileRoute("/forgot-password")({
  head: () => ({ meta: [{ title: "Reset password — ScamSniff" }, { name: "description", content: "Reset your ScamSniff password via email verification." }] }),
  component: Forgot,
});
        ...
        <FadeIn className="w-full">
          ...
          {!sent ? (
            <div className="clay-lg p-8">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Drop your email and we'll send a verification link to reset it.</p>
              ...
              <p className="mt-6 text-center text-sm text-muted-foreground">
                Remembered it? <Link to="/login" className="font-semibold text-[color:var(--primary)]">Back to login</Link>
              </p>
            </div>
          ) : (
            <div className="clay-lg p-8 text-center">
              <div className="mx-auto grid h-20 w-20 place-items-center rounded-3xl" style={{ background: "var(--clay-green)" }}>
                <MailCheck className="h-10 w-10" />
              </div>
              <h1 className="mt-6 font-display text-3xl font-bold">Check your inbox</h1>
              <p className="mt-2 text-sm text-muted-foreground">
                We sent a verification link. It expires in 30 minutes. Don't see it? Check spam — ironic, we know.
              </p>
              <button onClick={() => setSent(false)} className="clay-btn mt-6 px-5 py-2.5 text-sm font-semibold">Resend</button>
            </div>
          )}
        </FadeIn>
      </div>
