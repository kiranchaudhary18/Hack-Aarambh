...
import { 
  BookOpen, ShieldAlert, Sparkles, GraduationCap, Flame, ArrowRight, 
  HelpCircle, ShieldCheck, CheckCircle, Info, ChevronRight, Lock, Check, Zap
} from "lucide-react";
...
const scamPatterns = [
  { name: "Advance-fee fraud", desc: "You're asked to pay for 'training,' 'equipment,' or 'background checks' before starting.", color: "bg-rose-500/10 text-rose-600 border-rose-100/40", quote: "\"Send $50 setup fee via Bitcoin...\"" },
  { name: "Brand impersonation", desc: "Fake 'recruiters' use lookalike domains (amaz0n.com, g00gle.com) and stolen logos.", color: "bg-blue-500/10 text-blue-600 border-blue-100/40", quote: "\"Recruiting from global-apple.hr@gmail.com...\"" },
  { name: "Overpayment check", desc: "They send a fake check for 'supplies,' ask you to wire the difference. Check bounces. You're out.", color: "bg-amber-500/10 text-amber-600 border-amber-100/40", quote: "\"We sent you a $3,000 check for Macbooks...\"" },
  { name: "Reshipping mule", desc: "You receive packages, then reship them abroad. You're a money-laundering middleman.", color: "bg-emerald-500/10 text-emerald-600 border-emerald-100/40", quote: "\"Inspect these packages and ship to Turkey...\"" },
  { name: "Crypto wallet activation", desc: "Sign-on bonus paid in crypto — but only after you 'activate' your wallet with your own funds.", color: "bg-purple-500/10 text-purple-600 border-purple-100/40", quote: "\"Deposit 0.02 ETH to unlock sign-on reward...\"" },
  { name: "Data-harvesting interview", desc: "'Onboarding form' collects SSN, bank, driver's license before any work is offered.", color: "bg-orange-500/10 text-orange-600 border-orange-100/40", quote: "\"Upload SSN and routing bank details to start...\"" },
];
...
function Awareness() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (blobs) {
        gsap.to(blobs[0], { x: "12vw", y: "15vh", duration: 25, repeat: -1, yoyo: true, ease: "sine.inOut" });
        gsap.to(blobs[1], { x: "-15vw", y: "-12vh", duration: 28, repeat: -1, yoyo: true, ease: "sine.inOut" });
      }
  ...
  return (
    <div ref={awarenessRef} className="relative h-screen overflow-hidden bg-[oklch(0.97_0.018_95)] font-space">
          ...
          <div className="awareness-fade space-y-6">
            ...
            <header className="flex flex-col space-y-2 relative">
              ...
              <h1 className="text-3xl sm:text-4xl font-extrabold text-[oklch(0.24_0.04_270)] tracking-tight leading-tight">
                ...
                <span className="text-gradient animate-[pulse_4s_infinite]">spot them yourself.</span>
              </h1>
              <p className="text-xs sm:text-sm text-muted-foreground font-sans max-w-xl leading-relaxed mt-1">
                A growing field guide to the advanced fraud tactics targeting job seekers in 2026. Read once, recognize instantly, stay secure forever.
              </p>
            </header>
                ...
                <div className="space-y-2 max-w-xl font-sans">
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-0.5 rounded bg-rose-50 border border-rose-100 text-[9px] font-bold text-rose-600 uppercase tracking-wider shadow-sm flex items-center gap-1">
                      <Flame className="h-3 w-3 text-rose-500 animate-pulse" /> Critical Alert Pattern
                    </span>
                    <span className="text-[10px] text-muted-foreground font-semibold">Updated 4 hours ago</span>
                  </div>
                  ...
                  <p className="text-xs text-[oklch(0.4_0.03_270)] font-medium leading-relaxed">
                    Scammers send realistic PDFs impersonating Figma or Notion labs. They request that you purchase equipment (MacBooks) from a specified "verified corporate vendor link" and promise instant sign-on check reimbursement. The check is counterfeit.
                  </p>
                </div>
            ...
            <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr] items-start">
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <h2 className="text-lg font-extrabold text-[oklch(0.24_0.04_270)] font-space">Audit Radar · Six Red Flags</h2>
                </div>

                <div className="grid gap-4.5 sm:grid-cols-2">
                  {tips.map((t) => {
                    const Icon = (Icons as unknown as Record<string, React.ComponentType<{ className?: string; strokeWidth?: number }>>)[t.icon] || Sparkles;
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="grid h-12.5 w-12.5 place-items-center rounded-2xl shadow-[inset_0_1.5px_2px_rgba(255,255,255,0.9)]" style={{ background: `var(--${t.color})` }}>
                        ...
                        <h3 className="mt-4 font-space text-sm sm:text-base font-extrabold text-[oklch(0.24_0.04_270)] group-hover:text-[oklch(0.62_0.18_295)] transition-colors duration-300">{t.title}</h3>
                        <p className="mt-2 font-sans text-xs text-muted-foreground leading-relaxed">{t.desc}</p>
                        
                        {/* Smooth Expandable real-world examples (Interactive learning) */}
                        <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? "max-h-[180px] mt-4 pt-3.5 border-t border-[oklch(0.95_0.01_95)]" : "max-h-0"}`}>
                          <div className="space-y-2.5 font-sans text-[11px] text-[oklch(0.3_0.03_270)]">
                            <p className="bg-rose-50/50 p-2 rounded-xl border border-rose-100">
                              <strong>Classic Phrase:</strong> "We require all selected remote recruits to submit fee verifications before Day 1."
                            </p>
                        ...
                        <div className="mt-4 flex items-center justify-between text-[9px] font-extrabold uppercase font-space tracking-widest text-[oklch(0.62_0.18_295)] pl-0.5">
                          ...
                          <ChevronRight className={`h-4 w-4 transition-transform duration-300 ${isOpen ? "rotate-90 text-[oklch(0.62_0.18_295)]" : "text-muted-foreground"}`} />
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="space-y-5">
                ...
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl">
                  <div className="flex items-center gap-2.5 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <div>
                      <h4 className="text-xs font-extrabold uppercase text-[oklch(0.24_0.04_270)] font-space">Security Intel Index</h4>
                      <p className="text-[9px] text-muted-foreground font-sans">Common threat methods targeting talent segments.</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 space-y-3 font-sans text-[11px] text-[oklch(0.3_0.03_270)]">
                    {[
                      ...
                      <div key={idx} className="flex justify-between font-semibold">
                        ...
                        <span className="font-extrabold text-rose-500 shrink-0">{pattern.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl">
                  <div className="flex items-center gap-2.5 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <div>
                      <h4 className="text-xs font-extrabold uppercase text-[oklch(0.24_0.04_270)] font-space">Common Fraud Profiles</h4>
                      <p className="text-[9px] text-muted-foreground font-sans">Visual matrix cataloguing recruiter traps.</p>
                    </div>
                  </div>
            ...
            <div className="bg-white/75 border border-white/80 rounded-[40px] p-6 sm:p-8 shadow-[0_20px_50px_rgba(180,160,200,0.08),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl grid gap-6 md:grid-cols-[1.3fr_1fr] md:items-center">
              <div>
                <h2 className="text-xl sm:text-2xl font-extrabold text-[oklch(0.24_0.04_270)] font-space">The 3-Second Audit Rule</h2>
                <p className="mt-2 text-xs sm:text-sm text-muted-foreground font-sans leading-relaxed">
                  Before replying to any outreach message, click, or invitation, ask yourself three security verification questions:
                </p>
                
                <ul className="mt-5 space-y-3.5 font-sans text-xs sm:text-[13px] text-[oklch(0.3_0.03_270)] font-semibold">
                  <li className="flex items-start gap-2.5">
                    <span className="grid h-5 w-5 shrink-0 place-items-center rounded bg-purple-50 text-[10px] font-bold text-[oklch(0.62_0.18_295)] border border-[oklch(0.62_0.18_295/0.1)]">1</span>
                    <span>Does the sender's email domain match the exact corporate website registry? (Not globalhire@gmail.com)</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="grid h-5 w-5 shrink-0 place-items-center rounded bg-purple-50 text-[10px] font-bold text-[oklch(0.62_0.18_295)] border border-[oklch(0.62_0.18_295/0.1)]">2</span>
                    <span>Did you initiate an application process, or did this offer appear unprompted?</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="grid h-5 w-5 shrink-0 place-items-center rounded bg-purple-50 text-[10px] font-bold text-[oklch(0.62_0.18_295)] border border-[oklch(0.62_0.18_295/0.1)]">3</span>
                    <span>Are they demanding upfront payments, crypto, hardware vendor purchases, or sensitive credentials before live interviews?</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="clay-inset grid place-items-center p-8 text-center rounded-[32px] border border-white/60 bg-[oklch(0.95_0.01_95)]">
                <p className="font-space text-5xl sm:text-6xl font-extrabold text-gradient leading-none">3s</p>
                <p className="mt-2 text-xs font-bold text-[oklch(0.24_0.04_270)] font-space uppercase tracking-wider">All it takes to dodge a scam</p>
                <p className="text-[10px] text-muted-foreground font-sans mt-0.5">Protecting assets is seconds away.</p>
              </div>
            </div>
