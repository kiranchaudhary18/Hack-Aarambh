...
export const Route = createFileRoute("/report")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Submit a scam job offer you've received. Helps train our AI and protect future job seekers." },
  ]}),
  component: Report,
});
...
function Report() {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (jobText.trim().length < 30) return toast.error("Please paste at least 30 characters of the offer.");
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <section className="relative mx-auto w-[min(960px,94%)] pt-12">
        <FadeIn>
          ...
          <h1 className="mt-4 font-display text-5xl font-bold sm:text-6xl">Saw a scam? <span className="text-gradient">Tell us.</span></h1>
          <p className="mt-4 max-w-xl text-muted-foreground">
            Your reports help us train our detection model and warn other job seekers about emerging fraud patterns.
          </p>
        </FadeIn>
        ...
        <FadeIn delay={0.1}>
          <form onSubmit={submit} className="clay-lg mt-10 space-y-5 p-8">
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-2xl" style={{ background: "var(--clay-pink)" }}>
                <ShieldAlert className="h-6 w-6" />
              </div>
              <div>
                <p className="font-display text-lg font-bold">Submit suspicious offer</p>
                <p className="text-xs text-muted-foreground">Anonymous · used for ML training only</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {["Advance-fee", "Fake HR call", "Telegram job", "Brand impersonation", "Crypto wallet", "Other"].map((t) => (
                  <button type="button" key={t} onClick={() => setType(t)} className={`rounded-full px-4 py-2 text-xs font-semibold transition ${type === t ? "clay-primary" : "clay-btn"}`}>{t}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="text-sm font-semibold">Description <span className="text-muted-foreground">(optional)</span></label>
              <textarea
                value={desc}
