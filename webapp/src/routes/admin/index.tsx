...
import { adminStats, recentChecks, trendData } from "@/lib/mockData";
import { ScanSearch, ShieldAlert, Users, DollarSign, Flag, TrendingUp, ArrowRight, Activity, Radio, Globe2, Server, Cpu } from "lucide-react";
import { ResponsiveContainer, AreaChart, Area, Tooltip, XAxis, YAxis, CartesianGrid } from "recharts";

export const Route = createFileRoute("/admin/")({
  head: () => ({ meta: [{ title: "Admin — ScamSniff" }, { name: "description", content: "System overview and moderation dashboard." }] }),
  component: AdminHome,
});
      ...
      <div className="relative mx-auto flex h-full max-w-[1380px] gap-6 p-6">
        ...
        <main className="hide-scrollbar min-w-0 flex-1 space-y-6 overflow-y-auto pr-2 pb-6">
          <FadeIn>
            ...
            <p className="mt-2 text-muted-foreground">Live moderation stats across all scanners. Updated 12s ago.</p>
          </FadeIn>

          <StaggerChildren className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Stat icon={ScanSearch} label="Total scans" value={adminStats.totalScans.toLocaleString()} sub="+412 today" color="var(--clay-purple)" />
            <Stat icon={ShieldAlert} label="Scams detected" value={adminStats.scamsDetected.toLocaleString()} sub={`${Math.round(adminStats.scamsDetected / adminStats.totalScans * 100)}% rate`} color="var(--clay-pink)" />
            <Stat icon={Users} label="Active users" value={adminStats.activeUsers.toLocaleString()} sub="+86 this week" color="var(--clay-green)" />
            <Stat icon={DollarSign} label="Saved (est.)" value={`$${(adminStats.savedDollars / 1_000_000).toFixed(2)}M`} sub="Across all users" color="var(--clay-orange)" />
          </StaggerChildren>
            ...
            <FadeIn delay={0.1}>
              <div className="clay p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="clay-pill inline-flex items-center gap-1.5"><TrendingUp className="h-3 w-3" /> +24%</span>
                </div>
                <div className="mt-4 h-64">
                  <ResponsiveContainer>
                    <AreaChart data={trendData}>
                      ...
                      <Area type="monotone" dataKey="safe" stroke="oklch(0.72 0.16 155)" fill="url(#g2)" strokeWidth={3} />
                      <Area type="monotone" dataKey="scams" stroke="oklch(0.62 0.18 295)" fill="url(#g1)" strokeWidth={3} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <FadeIn delay={0.15}>
              <div className="clay flex h-full flex-col p-6">
                <div className="flex items-center justify-between">
                  ...
                  <Link to="/admin/flagged" className="text-sm font-semibold text-[color:var(--primary)]">All <ArrowRight className="ml-1 inline h-3 w-3" /></Link>
                </div>
                <div className="mt-4 space-y-3">
                  {recentChecks.filter((c) => c.score >= 70).slice(0, 4).map((c) => (
                    <Link to="/result" search={{ id: c.id }} key={c.id} className="clay-sm flex items-center gap-3 p-3 transition hover:-translate-y-0.5">
                      ...
                      <span className="clay-pill bg-[color:var(--destructive)] text-destructive-foreground">{c.score}%</span>
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <FadeIn delay={0.2}>
              <div className="clay p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="clay-pill inline-flex items-center gap-1.5" style={{ background: "var(--clay-green)" }}>
                ...
                <ul className="mt-4 space-y-3 text-sm">
                  {[
                    ...
                    <li key={i} className="flex items-start gap-3">
                      <span className="mt-1 grid h-7 w-7 shrink-0 place-items-center rounded-lg" style={{ background: x.c }}>
            ...
            <FadeIn delay={0.3}>
              <div className="clay p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="clay-pill text-[10px]" style={{ background: "var(--clay-green)" }}>All green</span>
                </div>
                <div className="mt-4 space-y-3">
                  {[
                    ...
                    return (
                      <div key={x.l} className="clay-inset flex items-center gap-3 p-3">
                        <span className="grid h-9 w-9 place-items-center rounded-xl" style={{ background: x.c }}>
