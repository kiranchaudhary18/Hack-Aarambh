...
import { 
  Camera, X, User, Mail, Bell, Trash2, LogOut, ShieldCheck, 
  KeyRound, CreditCard, Sparkles, Terminal, Activity, ArrowRight,
  Shield, CheckCircle, Info, Lock, Zap
} from "lucide-react";
...
export const Route = createFileRoute("/profile")({
  head: () => ({ 
    meta: [
      ...
      { name: "description", content: "Manage your ScamSniff neural protection profile parameters." }
    ] 
  }),
  component: Profile,
});
...
function Profile() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (blobs) {
        gsap.to(blobs[0], { x: "15vw", y: "10vh", duration: 25, repeat: -1, yoyo: true, ease: "sine.inOut" });
        gsap.to(blobs[1], { x: "-10vw", y: "-15vh", duration: 28, repeat: -1, yoyo: true, ease: "sine.inOut" });
      }
  ...
  return (
    <div ref={profileRef} className="relative h-screen overflow-hidden bg-[oklch(0.97_0.018_95)] font-space">
          ...
          <div className="profile-fade space-y-5 lg:space-y-6">
            ...
            <header className="flex flex-col space-y-2 relative">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground font-sans max-w-xl leading-relaxed">
                Manage your profile parameters, billing details, active plan restrictions, and live security warnings.
              </p>
            </header>
              ...
              <div className="bg-white/75 border border-white/80 rounded-[40px] p-6 sm:p-8 shadow-[0_20px_50px_-12px_rgba(150,130,180,0.15),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl flex flex-col items-center text-center relative overflow-hidden group">
                ...
                <div className="group relative transition-transform duration-300 hover:scale-[1.03] select-none">
                  <div
                    ...
                    style={{ 
                      background: photo ? undefined : "linear-gradient(135deg, oklch(0.68 0.16 295), oklch(0.55 0.22 305))" 
                    }}
                  >
                    {photo ? (
                ...
                <h2 className="mt-5 font-space text-xl sm:text-2xl font-extrabold text-[oklch(0.24_0.04_270)]">Aisha Khan</h2>
                <p className="text-xs text-muted-foreground font-sans font-semibold mt-0.5">aisha@scamsniff.ai</p>
                ...
                <span className="px-3.5 py-1 rounded-full bg-emerald-50 text-[10px] font-extrabold uppercase text-emerald-700 border border-emerald-500/10 flex items-center gap-1 mt-4 shadow-sm">
                  <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" /> Verified Member · Free plan
                </span>

                {/* Plan Scans usage visual block */}
                <div className="clay-inset mt-6 w-full p-4.5 text-left border border-white/60 relative">
                  <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground font-space">Audit telemetry scope</p>
                  <p className="mt-1 font-space text-2xl font-extrabold text-[oklch(0.24_0.04_270)]">12 <span className="text-xs text-muted-foreground font-sans font-semibold">/ 20 scans this month</span></p>
                  ...
                  <p className="text-[9px] text-muted-foreground font-sans font-semibold mt-2.5">Sync cycle resets on June 1, 2026.</p>
                </div>
              ...
              <div className="space-y-5">
                ...
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 sm:p-6 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl">
                  <div className="flex items-center gap-2 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <h3 className="text-sm font-extrabold uppercase text-[oklch(0.24_0.04_270)] font-space">Security parameters</h3>
                  </div>

                  <div className="mt-5 space-y-4">
                    <Row icon={User} label="Full name" value="Aisha Khan" />
                    <Row icon={Mail} label="Email address" value="aisha@scamsniff.ai" />
                    <Row icon={KeyRound} label="Password status" value="••••••••••" action="Update Credentials" />
                    <Row icon={CreditCard} label="Billing methods" value="No credit card active" action="Add Credit Card" />
                  </div>
                </div>
                ...
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 sm:p-6 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl">
                  <div className="flex items-center gap-2 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <h3 className="text-sm font-extrabold uppercase text-[oklch(0.24_0.04_270)] font-space">Intelligence logs alerts</h3>
                  </div>
                  ...
                  <div className="mt-4 space-y-2">
                    {[
                      ...
                      <label key={idx} className="flex items-center justify-between rounded-2xl py-2 cursor-pointer group">
                ...
                <div className="bg-rose-50/50 border border-rose-100 rounded-[32px] p-5 shadow-[0_15px_35px_rgba(254,226,226,0.1),_inset_0_2px_4px_rgba(255,255,255,0.9)] backdrop-blur-xl">
                  <div className="flex items-center gap-2 pb-3 border-b border-rose-100/60">
                    ...
                    <h3 className="text-sm font-extrabold uppercase text-rose-800 font-space">Danger center</h3>
                  </div>
  ...
  return (
    <button
      ...
      className={`relative h-6.5 w-11 rounded-full transition-colors duration-300 cursor-pointer p-[1.5px] ${
        isOn ? "bg-[oklch(0.62_0.18_295)]" : "bg-gray-200/80 shadow-[inset_1px_1px_2px_rgba(0,0,0,0.1)]"
      }`}
    >
      <span 
