    ...
    const fetchHistory = async () => {
      try {
        ...
        const transformedData = data.map((item: any) => ({
          ...
          verdict: (item.result?.isFake ? "scam" : "safe") as Verdict,
          reasons:
    ...
    return (
      <div className="relative h-screen overflow-hidden bg-[oklch(0.97_0.018_95)] font-space flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm font-bold text-muted-foreground">Loading audit history...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="archive-fade space-y-5 lg:space-y-6">
            ...
            <header className="flex flex-col space-y-2 relative">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-muted-foreground font-sans">
                <span>
                  Total resolved: <strong>{recentChecks.length} cases</strong>
                </span>
                <span className="h-3.5 w-px bg-gray-300" />
                <span>
                  Scams Caught:{" "}
                  <strong>{recentChecks.filter((c) => c.verdict === "scam").length} files</strong>
                </span>
                <span className="h-3.5 w-px bg-gray-300" />
                <span>
                  Protection accuracy: <strong>99.8%</strong>
                </span>
              </div>
            </header>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => {
                    ...
                    toast.success(
                      `Active filter: ${activeDateFilter === "all-time" ? "This Week" : "All Time"}`,
                    );
                  }}
              ...
              <div className="space-y-4">
                {view === "list" ? (
                  ...
                  <div className="grid gap-4.5 sm:grid-cols-3 items-start">
                    <KanbanCol
                      verdict="scam"
                      items={searched.filter((c) => c.verdict === "scam")}
                    />
                    <KanbanCol
                      verdict="suspicious"
                    ...
                    <KanbanCol
                      verdict="safe"
                      items={searched.filter((c) => c.verdict === "safe")}
                    />
                  </div>
                )}
              </div>
                ...
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl">
                  <div className="flex items-center gap-2.5 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <div>
                      <h4 className="text-xs font-extrabold uppercase text-[oklch(0.24_0.04_270)] font-space">
                        Telemetry Audit Rates
                      </h4>
                      <p className="text-[9px] text-muted-foreground font-sans">
                        Active threat indices tracked today.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 space-y-3.5 font-sans text-[11px] text-[oklch(0.3_0.03_270)]">
                    {[
                      ...
                      {
                        l: "Average verification score",
                        v: "92% integrity",
                        c: "text-emerald-600",
                      },
                      {
                        l: "Total audited files size",
                        v: "18.4 MB PDF/text",
                        c: "text-[oklch(0.24_0.04_270)]",
                      },
                    ].map((row, idx) => (
                ...
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl space-y-4">
                  <div className="flex items-center gap-2.5 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-muted-foreground font-sans">
                        Safety metrics issued by the AI copilot.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-purple-50/50 rounded-2xl border border-[oklch(0.62_0.18_295/0.1)] flex items-start gap-2.5">
                    ...
                    <p className="text-[11px] text-[oklch(0.3_0.03_270)] font-sans leading-relaxed">
                      <strong>Audit logs verified:</strong> 100% of your safe verified hires have
                      structured process paths. Avoid off-platform WhatsApp requests.
                    </p>
                  </div>
                </div>
              </div>
...
function EmptyState() {
  return (
    <EmptyStateComponent
      ...
      title="No matching audit files resolved"
      description="Try another company name, title, or search criteria."
    />
  );
}
...
function toneFor(v: Verdict) {
  return v === "scam"
    ? "bg-rose-500/10 text-rose-600 border-rose-100/40"
    : v === "suspicious"
      ? "bg-amber-500/10 text-amber-600 border-amber-100/40"
      : "bg-emerald-500/10 text-emerald-600 border-emerald-100/40";
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <span
          className={`grid h-12.5 w-12.5 shrink-0 place-items-center rounded-2xl border shadow-sm ${toneClass}`}
        >
          {c.source === "pdf" ? (
        ...
        <div className="min-w-0 flex-1 font-sans">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="font-space text-sm sm:text-base font-extrabold text-[oklch(0.24_0.04_270)] group-hover:text-[oklch(0.62_0.18_295)] transition-colors duration-300">
              {c.title}
            </h3>
            <span className="px-2 py-0.5 rounded bg-gray-100 text-[8px] font-bold text-muted-foreground uppercase tracking-wider border border-gray-200 shadow-sm">
              {c.source}
            </span>
          </div>
          ...
          <p className="mt-2 line-clamp-1 text-xs text-[oklch(0.4_0.03_270)] font-medium bg-gray-50/50 p-1.5 px-2.5 rounded-lg border border-gray-100/50">
            {c.snippet}
          </p>
        </div>
      </div>
    ...
    <div className="bg-white/75 border border-white/80 rounded-[32px] p-4 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl flex flex-col max-h-[calc(100vh-17.5rem)] relative overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-2 pb-3 border-b border-[oklch(0.95_0.01_95)]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-space text-xs font-extrabold uppercase text-[oklch(0.24_0.04_270)] tracking-wide">
              {meta.label}
            </p>
            <p className="text-[9px] text-muted-foreground font-sans font-semibold leading-tight">
              {meta.desc}
            </p>
          </div>
        </div>
        <span className="px-2 py-0.5 rounded bg-gray-100 text-[9px] font-bold text-muted-foreground font-space border border-gray-200 shadow-sm">
          {items.length}
        </span>
      </div>
        ...
        {items.map((c) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="font-space text-base font-extrabold text-[oklch(0.24_0.04_270)]">
                {c.score}%
              </span>
            </div>

            <h4 className="mt-2.5 line-clamp-2 font-space text-xs font-bold leading-relaxed text-[oklch(0.24_0.04_270)] group-hover:text-[oklch(0.62_0.18_295)] transition-colors duration-300">
              {c.title}
            </h4>
            <p className="mt-1 truncate text-[9.5px] text-muted-foreground font-semibold font-sans">
              {c.company} · {c.date}
            </p>
