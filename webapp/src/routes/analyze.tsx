...
export const Route = createFileRoute("/analyze")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Verify job offer legitimacy using neural threat models.",
      },
    ],
  }),
  component: Analyze,
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      if (tab === "pdf" && file) {
        ...
        payload = `PDF: ${file.name}`;
      } else {
        payload = tab === "text" ? text : tab === "url" ? `Job link: ${url}` : "";
        result = await api.analyzeText(payload || "Empty offer submission");
      }
      ...
      const transformedResult = {
        ...
        snippet: (typeof payload === "string" ? payload : "").slice(0, 160),
        date: new Date().toISOString().slice(0, 10),
        score: result.score,
        verdict: (result.isFake ? "scam" : "safe") as "scam" | "safe" | "suspicious",
        reasons:
          result.reasons?.map((r: string) => ({
            label: r,
            severity: (result.isFake ? "high" : "low") as "high" | "med" | "low",
            detail: r,
          })) || [],
        source: tab as "text" | "pdf" | "url",
      };

      resultStore.set(transformedResult);
          ...
          {loading ? (
            ...
            <div className="h-full w-full bg-white/75 backdrop-blur-xl border border-white/80 rounded-[40px] shadow-[0_20px_50px_rgba(180,160,200,0.12)] p-8 flex flex-col items-center justify-center space-y-8 animate-[fade-in_0.3s_ease-out]">
              ...
              <div className="text-center space-y-2.5 max-w-sm">
                <h3 className="text-sm font-extrabold uppercase tracking-widest text-[oklch(0.24_0.04_270)] font-space">
                  Audit telemetry running
                </h3>
            ...
            <div className="workspace-fade space-y-5 lg:space-y-6">
              ...
              <header className="flex flex-col space-y-2 relative">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground font-sans max-w-xl leading-relaxed">
                  Paste the suspicious message, upload an offer PDF, or share the URL. Our threat
                  engines evaluate indicators and issue an audit score in under 4 seconds.
                </p>
              </header>
              ...
              <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr] items-start">
                    ...
                    {tab === "text" && (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-1.5 text-[9.5px] font-bold pl-1.5 text-muted-foreground uppercase font-space tracking-wide">
                          ...
                          <span>
                            Tip: Include salaries and payment descriptions for higher confidence
                            models.
                          </span>
                        </div>
                      </div>
                    ...
                    {tab === "pdf" && (
                      <label className="clay-inset flex cursor-pointer flex-col items-center justify-center gap-3.5 p-10 text-center border-2 border-dashed border-[oklch(0.62_0.18_295/0.2)] hover:border-[oklch(0.62_0.18_295/0.5)] hover:shadow-lg transition-all duration-300 group">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="px-2 py-0.5 rounded bg-purple-50 text-[8px] font-bold text-[oklch(0.62_0.18_295)] uppercase tracking-wider border border-[oklch(0.62_0.18_295/0.1)]">
                            PDF FORMAT
                          </span>
                          <span className="px-2 py-0.5 rounded bg-purple-50 text-[8px] font-bold text-[oklch(0.62_0.18_295)] uppercase tracking-wider border border-[oklch(0.62_0.18_295/0.1)]">
                            MAX 10MB
                    ...
                    {tab === "url" && (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-1.5 text-[9.5px] font-bold pl-1.5 text-muted-foreground uppercase font-space tracking-wide">
                          ...
                          <span>
                            Link crawler checks SSL certificates, DNS registry age, and spoofed
                            domains.
                          </span>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2.5 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] text-muted-foreground font-sans">
                          Common scam patterns observed this week.
                        </p>
                      </div>
                    </div>
                      ...
                      {[
                        {
                          ...
                          icon: ShieldAlert,
                          desc: "Recruiters asking for processing, software setup, or onboarding fees prior to work contracts.",
                        },
                        {
                          title: "Public Sender Domains",
                          icon: Info,
                          desc: "Communication extending from free web mail hosting services (e.g. gmail.com, mail.com, outlook.com) is spoofed.",
                        },
                      ].map((pattern, idx) => (
                        <div key={idx} className="flex gap-3">
                          ...
                          <div className="text-[11px] text-[oklch(0.3_0.03_270)]">
                            <p className="font-extrabold text-[oklch(0.24_0.04_270)]">
                              {pattern.title}
                            </p>
                  ...
                  <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl relative overflow-hidden flex items-center justify-between">
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground font-space">
                        Secure Database
                      </h4>
                      <p className="text-base font-extrabold text-[oklch(0.24_0.04_270)] font-space mt-1">
                        291,482 Blacklists
                      </p>
                    </div>
              ...
              {tab === "text" && (
                <div className="bg-white/75 border border-white/80 rounded-[32px] p-5 sm:p-6 shadow-[0_15px_35px_rgba(180,160,200,0.06),_inset_0_2px_4px_rgba(255,255,255,0.95)] backdrop-blur-xl">
                  <div className="flex items-center gap-3 pb-3 border-b border-[oklch(0.95_0.01_95)]">
                    ...
                    <div>
                      <h2 className="text-sm font-extrabold text-[oklch(0.24_0.04_270)] font-space">
                        Simulated Training Vectors
                      </h2>
                      <p className="text-[9.5px] text-muted-foreground font-sans">
                        Click a training model template below to load details into workspace.
                      </p>
                    </div>
                  </div>
