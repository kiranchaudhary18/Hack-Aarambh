...
export const Route = createFileRoute("/analyze")({
  head: () => ({ meta: [{ title: "Analyze offer — ScamSniff" }, { name: "description", content: "Paste, upload, or link a job offer for instant scam analysis." }] }),
  component: Analyze,
});
...
function Analyze() {
  ...
  function handleAnalyze() {
    if (tab === "text" && text.trim().length < 20) return toast.error("Paste at least 20 characters of the offer.");
    if (tab === "text" && text.length > 8000) return toast.error("Text must be under 8000 characters.");
    if (tab === "url" && !/^https?:\/\/.+\..+/.test(url.trim())) return toast.error("Enter a valid URL starting with http(s)://");
    if (tab === "pdf") {
      ...
      <div className="relative mx-auto flex h-full max-w-[1380px] gap-6 p-6">
        ...
        <main className="hide-scrollbar min-w-0 flex-1 space-y-6 overflow-y-auto pr-2 pb-6">
          ...
          <FadeIn>
            ...
            <h1 className="mt-3 font-display text-4xl font-bold sm:text-5xl">Drop the offer in here.</h1>
            <p className="mt-2 text-muted-foreground">Paste the message, upload a PDF, or share the URL. We score it in seconds.</p>
          </FadeIn>
          ...
          <FadeIn delay={0.1}>
            <div className="clay-lg p-6 md:p-8">
              ...
              <div className="mt-6">
                ...
                {tab === "pdf" && (
                  <label className="clay-inset flex cursor-pointer flex-col items-center justify-center gap-3 p-12 text-center">
                    <span className="grid h-16 w-16 place-items-center rounded-3xl" style={{ background: "var(--clay-blue)" }}>
                      <Upload className="h-7 w-7" />
                    </span>
                    <p className="font-display text-xl font-bold">{file ? file.name : "Drop your offer PDF"}</p>
                    <p className="text-sm text-muted-foreground">{file ? `${(file.size / 1024).toFixed(0)} KB · click to replace` : "or click to browse · max 10MB"}</p>
              ...
              <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
                <p className="text-xs text-muted-foreground">We never store the raw content. Analysis runs in under 4 seconds.</p>
                <button
            ...
            <FadeIn delay={0.2}>
              <div className="clay p-6">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  {samples.map((s, i) => (
                    <button key={i} onClick={() => setText(s)} className="clay-sm p-4 text-left text-sm transition hover:-translate-y-0.5">
                      {s.slice(0, 110)}…
                    </button>
                  ))}
                </div>
