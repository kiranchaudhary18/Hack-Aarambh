...
export default function RegionScanner({ onLogout, language }: { onLogout: () => void; language: Language }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Scanner: Failed to attach listener:", err);
    }
  }, []);
  ...
  const handleScanArea = async () => {
    try {
      ...
      const browserAPI = getBrowserAPI();
      console.log("browserAPI object:", typeof browserAPI, browserAPI);
      console.log("browserAPI.tabs:", typeof browserAPI.tabs, browserAPI.tabs);
      ...
      // Inject content script via background service worker
